...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    ...
    body: (
      <>
        <p>
          Debate Flow is a keyboard-first tool for flowing competitive rounds.
          Everything you type stays on this device. Your flows live in your
          browser, never on a server, and they work offline.
        </p>
        <p>
          This guide covers the whole app, from the dashboard to flowing a live
          round. You can reopen it any time from the Guide button.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          The dashboard lists every flow you have saved. New flow creates one and
          opens the editor; click any card to reopen a round.
        </p>
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          A round is a set of sheets. Each sheet is a grid: columns are speeches
          in time order, and rows are arguments. You read an argument left to
          right across the speeches as it gets answered.
        </p>
        <GridDiagram />
        <p>
          A response sits in the next speech&rsquo;s column on the same row as the
          argument it answers. Aff and Neg sheets track each side&rsquo;s offense;
          CX sheets capture cross-examination.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          Arrow to a cell and type to flow an argument. The editor keeps the
          structure tidy as you go.
        </p>
        <ul className="ml-4 list-disc space-y-1">
          <li>
          ...
          <li>
            <Kbd cmd="node.response" /> adds a response in the next speech,
            answering the current argument.
          </li>
          <li>
            <Kbd cmd="row.insertAbove" /> and <Kbd cmd="row.insertBelow" /> add
            blank rows; <Kbd cmd="row.delete" /> removes a row and{" "}
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Arrow keys move between cells. Jump keys leap to the edge of the
          current block, and Home/End reach the ends of a row.
        </p>
        <ul className="ml-4 list-disc space-y-1">
          <li>
          ...
          <li>
            ...
            <Kbd cmd="sheet.next" />, or <Kbd cmd="sheet.quickSwitch" /> to jump to
            any sheet by name.
          </li>
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Info records the teams, schools, side, judge, and result for the round,
          the same fields the dashboard sorts and groups by.
        </p>
        <p>
          Settings is where you change display options and remap keys. Every
          shortcut shown in this guide reflects your current keymap.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          Your work saves automatically as you type, and the save indicator shows
          the status. Undo and redo step through changes.
        </p>
        <p>
          Export writes the round to xlsx or csv, and Print produces a clean
          printable flow.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          Debate Flow is built to run from the keyboard. Some actions are modal:
          grab an argument with <Kbd cmd="move.grab" />, move it with the arrow
          keys, then <Kbd cmd="move.commit" /> to drop it or{" "}
          <Kbd cmd="move.cancel" /> to abandon the move.
        </p>
        <p>
